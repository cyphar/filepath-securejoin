...
func setupProcOvermount(t *testing.T) {
	...
	require.NoError(t, err, "get real host /proc during setup")
	defer procRoot.Close()

	procThreadSelf, closer, err := procThreadSelf(procRoot, ".")
	require.NoError(t, err, "get real host /proc/thread-self during setup")
	...
	for _, mount := range []struct {
		...
		require.NoErrorf(t, err, "mount(%s, [%s/]%s, %s, 0x%x)", mount.source, procThreadSelfPath, mount.targetSubPath, mount.fsType, mount.flags)
	}
}
