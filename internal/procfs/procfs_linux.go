...
func (proc *Handle) readlink(base procfsBase, subpath string) (string, error) {
	...
	if err := fd.CheckSubpathOvermount(proc.Inner, link, ""); err != nil {
		// TODO: Once we bump the minimum Go version to 1.20, we can use
		// multiple %w verbs for this wrapping. For now we need to use a
		// compatibility shim for older Go versions.
		// err = fmt.Errorf("check safety of %s/%s: %w: %w", base, subpath, errUnsafeProcfs, err)
		err = gocompat.WrapBaseError(err, errUnsafeProcfs)
